 ...
 * @author kingyang
 */
public class QuizSharedPreferences extends ContentProvider implements SharedPreferences {
    private static final String TAG = "FaceSharedPreferences";
    ...
    private static class ReflectionUtil {
        ...
        public static ContentValues contentValuesNewInstance(HashMap<String, Object> values) {
            try {
                Constructor<ContentValues> c = ContentValues.class.getDeclaredConstructor(
                        HashMap.class); // hide
        ...
        public static Editor editorPutStringSet(Editor editor, String key, Set<String> values) {
            try {
                Method method = editor.getClass().getDeclaredMethod("putStringSet", String.class,
                        Set.class); // Android 3.0
    ...
    private void checkInitAuthority(Context context) {
        if (sAuthorityUrl == null) {
            synchronized (QuizSharedPreferences.this) {
                if (sAuthorityUrl == null) {
                    ...
                    if (packageInfos != null && packageInfos.providers != null) {
                        for (ProviderInfo providerInfo : packageInfos.providers) {
                            if (providerInfo.name.equals(QuizSharedPreferences.class.getName())) {
                    ...
                    if (sAuthoriry == null) {
                        throw new IllegalArgumentException("'AUTHORITY' initialize failed, Unable to find explicit provider class " + QuizSharedPreferences.class.getName() + "; have you declared this provider in your AndroidManifest.xml?");
                    }
     ...
     */
    public static QuizSharedPreferences getSharedPreferences(Context context, String name, int mode) {
        return new QuizSharedPreferences(context, name, mode);
    }

    public QuizSharedPreferences() {

    }

    private QuizSharedPreferences(Context context, String name, int mode) {
        mContext = context;
        mName = name;
        mMode = mode;
                            ...
                            if (mName.equals(name) && keysModified != null) {
                                ...
                                for (int i = keysModified.size() - 1; i >= 0; i--) {
                                    ...
                                    for (OnSharedPreferenceChangeListener listener : listeners) {
                                        if (listener != null) {
                                            listener.onSharedPreferenceChanged(QuizSharedPreferences.this, key);
                                        }
                                    }
                                }
                            }
    ...
    private String makeAction(String name) {
        return String.format("%1$s_%2$s", QuizSharedPreferences.class.getName(), name);
    }
